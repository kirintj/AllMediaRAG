...
- **生产级基础设施**：Redis Stream 异步任务队列、MinIO/S3 文件存储、JWT 认证、速率限制、结构化 JSON 日志、多租户隔离、8 容器 Docker Compose 一键部署
...
- **响应式跨端适配**：桌面端三栏布局，移动端底部导航 + 横向滚动标签 + 顶部 Sheet 抽屉，安全区与触摸目标优化
...
│   │   ├── documents.py                  # 文档上传 / 删除 / 批量加载 / 增量同步
│   │   ├── knowledgebases.py             # 知识库 CRUD 管理
│   │   ├── tag_kb.py                     # 标签知识库管理
│   │   ├── graph.py                      # 知识图谱查询接口
│   │   ├── models.py                     # LLM / Embedding 模型管理
│   │   ├── settings.py                   # 系统设置接口
│   │   ├── team.py                       # 团队管理接口
│   │   ├── eval.py                       # 评测报告查询接口
│   │   └── deps.py                       # API 依赖注入（数据库会话、当前用户）
│   ├── core/                             # 核心业务模块
│   │   ├── config.py                     # 统一配置管理（AppSettings）
│   │   ├── rag_engine.py                 # RAG 引擎门面（三服务编排）
│   │   ├── llm_client.py                 # LLMBundle（统一 llm / embedding 调用）
│   │   ├── document_processor.py         # 文档解析、OCR、分块、LLM 增强
│   │   ├── image_store.py                # 原图文件系统存储（MD5 去重）
│   │   ├── bm25_retriever.py             # BM25 关键词检索（jieba + 持久化）
│   │   ├── auth.py                       # JWT 认证核心逻辑
│   │   ├── index_manager.py              # 索引状态管理
│   │   ├── rate_limit.py                 # 速率限制
│   │   ├── services/                     # 四服务架构
│   │   │   ├── infra_bundle.py           # InfraBundle 共享依赖容器
...
│   │   ├── models/                       # 数据模型
│   │   │   ├── embedding_providers.py    # Embedding Provider 实现（OpenAI / Ollama / Tongyi / SiliconFlow / HuggingFace）
│   │   │   ├── llm_bundle.py             # LLM 统一调用封装
│   │   │   ├── chat_providers.py         # Chat Provider 实现
│   │   │   ├── rerank_providers.py       # Rerank Provider 实现
│   │   │   ├── ocr_providers.py          # OCR Provider 实现
│   │   │   ├── cv_providers.py           # CV Provider 实现
│   │   │   ├── asr_providers.py          # ASR Provider 实现
│   │   │   ├── tts_providers.py          # TTS Provider 实现
│   │   │   ├── tenant_llm_service.py     # 租户 LLM 服务管理
...
│   │   └── ...                           # 迁移、测试等脚本
│   └── alembic/                          # 数据库迁移
│       └── versions/                     # 迁移版本（pgvector / 表创建 / 对话增强）
├── frontend/                             # Vue 3 前端
│   ├── src/
│   │   ├── App.vue                       # 根组件（双栏布局 + 页面路由）
│   │   ├── main.js                       # 应用入口
│   │   ├── components/                   # 通用 UI 组件
...
│   │   │   └── settings/                # 系统设置
│   │   │       ├── SettingsOverview.vue   # 总览
│   │   │       ├── SettingsDocParsing.vue # 文档解析
│   │   │       ├── SettingsRaptor.vue    # RAPTOR
│   │   │       ├── SettingsTagging.vue   # 内容标签
│   │   │       ├── SettingsGraphRAG.vue  # 知识图谱
...
│   │   │   └── useToastStore.js
│   │   ├── api/                          # Axios API 封装
│   │   │   ├── auth.js / chat.js / conversations.js / documents.js
│   │   │   ├── knowledgebases.js / tagKb.js / models.js / settings.js / graph.js / team.js / eval.js
│   │   │   └── index.js
│   │   ├── composables/                  # 组合式函数
│   │   ├── lib/                          # 工具库
│   │   │   └── utils.ts
...
│   └── vite.config.js
├── tests/                                # 测试套件
│   └── unit/                             # 单元测试（查询理解 / 重排序 / 分块 / 评测 / 缓存 / 服务层 / 知识图谱）
├── data/                                 # 运行时数据
...
├── docs/                                 # 设计与部署文档
│   └── superpowers/                      # 特性设计文档
├── docker-compose.yml                    # Docker 全栈编排（含 ES / Redis / PostgreSQL / Neo4j / MinIO）
├── Dockerfile                            # 后端镜像构建
...
| `/api/conversations` | GET/DELETE | 对话列表 / 清空 | 是 |
| `/api/conversations/{id}` | GET/DELETE | 对话详情 / 删除 | 是 |
| `/api/upload` | POST | 单文件上传 | 是 |
| `/api/upload/batch` | POST | 批量上传 | 是 |
| `/api/documents` | GET/DELETE | 文档列表 / 清空 | 是 |
| `/api/documents/load` | POST | 加载本地文档 | 是 |
| `/api/documents/sync` | POST | 增量同步 | 是 |
| `/api/knowledgebases` | CRUD | 知识库管理 | 是 |
| `/api/tag_kb` | CRUD | 标签知识库 | 是 |
| `/api/graph` | GET | 知识图谱查询 | 是 |
| `/api/models` | GET | 模型列表查询 | 是 |
| `/api/settings` | GET/PUT | 系统设置 | 是 |
| `/api/team` | CRUD | 团队管理 | 是 |
| `/api/stats` | GET | 系统统计 | 是 |
| `/api/metrics` | GET | 性能指标 | 是 |
| `/api/eval/reports` | GET | 评测报告 | 是 |
| `/health` | GET | 健康检查 | 否 |
...
1. 在 `backend/core/models/` 下实现新 Provider 类，设置 `_FACTORY_NAME`
2. 在 `__init__.py` 中注册或使用自动发现
